...
public ReadDownloads(){
	new String:file[256];
	BuildPath(Path_SM, file, 255, "configs/downloads_wcs.ini");
...
public ReadItem(String:buffer[]){
	...
	else if(StrContains(buffer,"//Particle Files (Download and Precache)",true) >= 0){
		strcopy(mediatype,255,"Particle");
		downloadfiles=true;
	}
	...
	else if (!StrEqual(buffer,"",false) && FileExists(buffer))
	{
		if(downloadfiles){
			...
			else if(StrContains(mediatype,"Particle",true) >= 0){
...
public ReadDownloadsSimple(){
	new String:file[256];
	BuildPath(Path_SM, file, 255, "configs/downloads_simple_wcs.ini");
	new Handle:fileh = OpenFile(file, "r");
	new String:buffer[256];
	downloadtype = 2;
	new len;
